 ...
 */
class Category_Custom_Two extends \Tribe__Events__Filterbar__Filter {
	use Context_Filter;
	...
	protected function get_values() {
		 ...
		 */
		$wanted_categories = apply_filters( 'tec_labs_custom_category_filter_wanted_categories',  (array) $wanted_categories );
		
		// Preprocess the terms
		foreach ( $source as $term ) {
			// If the term is not in the wanted array, then skip it.
			if ( ! empty ( $wanted_categories ) ) {
				if ( ! in_array( $term->slug, $wanted_categories ) ) {
					continue;
				}
			}
