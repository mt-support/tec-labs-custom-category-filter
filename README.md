...
* `Custom Category Filter` - Plugin Human-readable name, e.g. "Sample Extension"
* `Custom_Category_Filter` - Which namespace we will use for the plugin. Usually the plugin name or the slug in uppercase without spaces, e.g. "SampleExtension"
* `custom-category-filter` - Uses using dashes normally, e.g. "sample-extension"
* `__TRIBE_DOMAIN__` - Translation domain normally with "tribe-" prefix, e.g. "tec-labs-sample-extension"
* `custom_category_filter` - Uses underscores, so it's safe for variables, e.g. "sample_extension"
